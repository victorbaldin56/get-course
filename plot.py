...
figure, axes = plt.subplots(figsize = (16, 10), dpi = 200)

axes.set_xlabel("Время, с")
axes.set_ylabel("Напряжение, В")
axes.set_title("Зарядка и разрядка RC-цепи")
...
charge_plot_line.set_label("Зарядка")
discharge_plot_line.set_label("Разрядка")
axes.legend()
...
axes.grid(color = "blue", which = "minor", linestyle = ':', linewidth = 0.5)
axes.grid(color = "blue", which = "major", linestyle = ":", linewidth = 1)

charge_time    = time_arr[volt_max_ind] - time_arr[0]
discharge_time = time_arr[-1] - time_arr[volt_max_ind]

axes.axvline(x = charge_time, ymin=y_limits[0], ymax = volt_max/y_limits[1], linestyle='dashed')
axes.axhline(y = volt_max, xmin=x_limits[0], xmax = charge_time/x_limits[1], linestyle='dashed')

axes.scatter(time_arr[volt_max_ind], volt_max)
...
axes.text(x = (charge_time/2-0.8), y = volt_max/2, s = ("Время зарядки: " + str(round(charge_time, 2)) + " с"), color = 'blue')
axes.text(x = (charge_time+discharge_time/2-0.8), y = volt_max/2, s = ("Время разрядки: " + str(round(discharge_time, 2)) + " с"), color = 'red')
